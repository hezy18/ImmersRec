...
class BaseRunner(object):
	...
	def evaluate(self, dataset: BaseModel.Dataset, topks: list, metrics: list) -> Dict[str, float]:
		...
		"""
		predictions,immersions = self.predict(dataset)
	...
	def predict(self, dataset: BaseModel.Dataset) -> np.ndarray:
		...
		dl = DataLoader(dataset, batch_size=self.eval_batch_size, shuffle=False, num_workers=self.num_workers,
						collate_fn=dataset.collate_batch, pin_memory=self.pin_memory)
		for batch in tqdm(dl, leave=False, ncols=100, mininterval=1, desc='Predict'): # desc='Epoch {:<3}'
			if hasattr(dataset.model,'inference'):
				result = dataset.model.inference(utils.batch_to_gpu(batch, dataset.model.device))
			else:
				result = dataset.model(utils.batch_to_gpu(batch, dataset.model.device))
			prediction = result['prediction']
			predictions.extend(prediction.cpu().data.numpy())
			if 'immersion' in result:
				immersion =result['immersion']
				immersions.extend(immersion.cpu().data.numpy())
			
			
			
		predictions = np.array(predictions)
		immersions = np.array(immersions)
