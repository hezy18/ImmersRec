...
class AutoInt(ContextModel):
	...
	def _define_params(self):
		...
		# Deep
		pre_size = self.context_feature_num * self.attention_size
		...
		for size in self.layers:
			...
			pre_size = size
		self.output_embeddings = nn.Linear( self.layers[-1], 1)
	...
	def forward(self, feed_dict):
		...
		# context_features = feed_dict['context_mh'] # Batch size * item num * field num
		...
		# autoint_all_embeddings = self.context_embedding(context_features) # Batch_size * item num * field num * embed dim
		...
		predictions = linear_value + self.output_embeddings(deep_vectors).squeeze()
