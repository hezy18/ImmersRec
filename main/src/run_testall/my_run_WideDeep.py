...
def WideDeep_command(data='KuaiRand'):
    script_content=''
    for emb_size in ['32','64','128']:
        for layer in ['\'[64]\'','\'[128]\'','\'[64,64]\'']:
            for _ in range(5):  
                ...
                command_list=['python','main.py', '--model_name', 'WideDeep', '--dataset', data, 
    ...
    with open(f'command_testall/run_{data}_WideDeep_baseline.sh','w') as f:
        ...
        for layer in ['\'[64]\'','\'[128]\'','\'[64,64]\'']:
            for _ in range(5):  
                ...
                for _ in range(5):
                    ...
                    script_content += content 
                    script_content += '\n'
    with open(f'command_testall/run_{data}_WideDeep_ImmersRec.sh','w') as f:
        f.write(script_content)
        
WideDeep_command('KuaiRand')
WideDeep_command('MicroVideo')
