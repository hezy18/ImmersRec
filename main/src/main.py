...
from net_model import Formula_Model
...
def parse_global_args(parser):
    ...
    parser.add_argument('--pretrained', type=int, default=0,
                        help="Wheter to load the pretrained model and fix the base param")
    parser.add_argument('--fixed_all', type=int, default=0,
                        help="Wheter to fix all param")
    parser.add_argument('--fixed_no', type=int, default=0,
                        help="Wheter to non-fix")
    return parser
...
def main():
    ...
    if args.pretrained > 0: # only for fix some parameters
        pretrained_model = torch.load('pretrained_model_allNor.pth',map_location=args.device)
        if not args.fixed_no:
    ...
    del data_dict[phase].corpus.context_feature_df, data_dict[phase].corpus.data_df
    for phase in ['train','dev','test']:
        data_dict[phase].prepare()
...
if __name__ == '__main__':
    ...
    for arg in ['tradeoff_DA', 'tradeoff_Clf','include_immersion'] + model_name.extra_log_args:
        log_args.append(arg + '=' + str(eval('args.' + arg)))
    log_file_name = '__'.join(log_args).replace(' ', '__')
    if args.log_file == '':
        args.log_file = '../log/{}/{}_repeat.txt'.format(init_args.model_name, log_file_name)
    if args.model_path == '':
        args.model_path = '../model/{}/{}_repeat.pt'.format(init_args.model_name, log_file_name)
